...
const multiSiteSchema = z.object({
...
type MultiSiteFormData = z.infer<typeof multiSiteSchema>;

interface MultiSiteSnowRemovalFormProps {
  onSubmit?: (data: CreateReportRequest[]) => Promise<void>;
  className?: string;
}

export function MultiSiteSnowRemovalForm({
  onSubmit,
  className,
}: MultiSiteSnowRemovalFormProps) {
  ...
  } = useForm<MultiSiteFormData>({
    resolver: zodResolver(multiSiteSchema),
    defaultValues: {
  ...
  useEffect(() => {
    ...
    const loadWeatherData = async () => {
      ...
      try {
        ...
        setWeatherError("Error loading weather data");
      } finally {
  ...
  const onFormSubmit = async (data: MultiSiteFormData) => {
    setLoading(true);
          ...
          <CardContent className="space-y-4">
            ...
            {weatherLoading && (
              <div className="flex items-center gap-2 text-sm text-muted-foreground">
                <CloudSnow className="h-4 w-4 animate-spin" />
